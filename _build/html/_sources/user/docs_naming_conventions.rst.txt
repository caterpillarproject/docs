...
Much of this pertains to using the cluster at MIT but it describes our overall naming convention used and how the zoom-in halos are linked back to the parent halos.
...
Every halo has its own directory For example, ``H1387186`` is located
here:
...
Here is the initial list of Caterpillar halos found in Griffen et al.
(2015) with their associated ID taken from the Rockstar catalogue of
halos for the parent simulation.
...
Most of the terms remain the same for the entire Caterpillar suite. The
only ones that do change are the ``LX`` value and the ``B`` value which
signifies the resolution and lagrangian geometry accordingly. ``NV``
also represents the number of times the virial radius we enclosed within
the parent volume (usually four, though sometimes 5).

All halos directories will have three key sub-directories:
``halos_bound/``, ``outputs/`` and ``analysis/``. In each of these
directors you'll find the following folders:
...
You will also notice some other folders, such as ``halos/`` which was
the original catalogues produced prior to our implementation of
iterative binding. This data can be used to explore the benefits of
using full iterative unbinding.

Ancillary Information
---------------------

At the top level of the halos directory
(``/bigbang/data/AnnaGroup/caterpillar/halos/``), you will find
``parent_zoom_index.txt``. This file contains the following kind of
table
...
This above contains *Caterpillar as it stands today*. The columns are as
follows:
...
Although it is not recommended, you can load up any of these quantities
by importing Brendan Griffen's modules:
